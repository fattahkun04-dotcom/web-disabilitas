...
export default function AdminKeuanganPage() {
  ...
  }>({ isOpen: false, txId: "", txDescription: "" });
  ...
  const fetchTransactions = async (page: number, month: string, year: string) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    fetchTransactions(pagination.page, selectedMonth, selectedYear);
  }, [pagination.page, selectedMonth, selectedYear]);

  const handlePageChange = (newPage: number) => {
    setPagination((prev) => ({ ...prev, page: newPage }));
  };

  const handleDelete = (txId: string, txDesc: string) => {
    setDeleteModal({ isOpen: true, txId, txDescription: txDesc });
  };

  const confirmDelete = async () => {
    const { txId, txDescription } = deleteModal;
    setDeleteModal((prev) => ({ ...prev, isOpen: false }));
    setActionLoading(txId);
    try {
      const res = await fetch(`/api/budget/${txId}`, {
        method: "DELETE",
      });
      if (res.ok) {
        toast({
          title: "Berhasil",
          description: `Transaksi "${txDescription}" telah dihapus.`,
          variant: "success",
        });
        fetchTransactions(pagination.page, selectedMonth, selectedYear);
        fetchBudgetSummary(selectedMonth, selectedYear);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Kelola pemasukan dan pengeluaran paguyuban
          </p>
        </div>
        <Link href="/admin/keuangan/baru">
          <Button>
                      ...
                      <tr key={tx.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center justify-end gap-1">
                            <Link href={`/admin/keuangan/${tx.id}/edit`}>
      ...
      <ConfirmModal
        ...
        message={`Yakin ingin menghapus transaksi "${deleteModal.txDescription}"? Tindakan ini tidak dapat dibatalkan.`}
