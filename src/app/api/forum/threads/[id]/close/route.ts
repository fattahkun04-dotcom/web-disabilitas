...
export async function POST(
  ...
  try {
    ...
    if (session.user.role !== "ADMIN" && session.user.role !== "SUPER_ADMIN") {
      return NextResponse.json(
        { success: false, error: { code: "FORBIDDEN", message: "Hanya admin yang dapat menutup thread" } },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      { success: false, error: { code: "INTERNAL_ERROR", message: "Failed to toggle close" } },
      { status: 500 }
    );
  }
}
